...
pub fn execute_suggestion_command(command: &str, variable_cache: &VariableCache) -> Result<String> {
    ...
    let child = cmd.spawn().map_err(|e| ShellSpawnError::new(command, e))?;

    let output = child
        .wait_with_output()
        .context("Failed to wait and collect output from shell command")?;

    String::from_utf8(output.stdout).context("Suggestion command output is not valid UTF-8")
}
...
pub fn apply_suggestion_options(
    ...
    if let Some(ref delimiter) = options.delimiter {
        preview_env_vars.insert(env_var::PREVIEW_DELIMITER.to_string(), delimiter.clone());
    }
